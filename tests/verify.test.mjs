...
try {
  ...
  runGit(repo, "-c", "user.email=test@example.invalid", "-c", "user.name=Test", "tag", "-a", "v0.1.8", "-m", "release");
  const annotatedTagObject = runGit(repo, "rev-parse", "refs/tags/v0.1.8");
  const annotatedTagCommit = runGit(repo, "rev-parse", "refs/tags/v0.1.8^{}");
...
assert.match(workflow, /releases\.ndjson/);
assert.match(workflow, /jq -s/);
assert.match(workflow, /wc -c <"\$release_pages_json"/);
assert.match(workflow, /ambiguous release records/);
assert.match(workflow, /draft release metadata does not match/);
assert.match(workflow, /release_id=.*jq/);
assert.match(workflow, /--hostname uploads\.github\.com --method POST/);
assert.match(workflow, /Content-Type: application\/octet-stream/);
assert.match(workflow, /releases\/\$release_id\/assets\?name=\$package_name/);
assert.match(workflow, /--hostname github\.com --method PATCH/);
